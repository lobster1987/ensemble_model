...
class BatchLoaderUnk:
    def __init__(self, tokens, data_dir, batch_size, seq_length, max_word_l, n_words, n_chars):
        ...
        print(type(self.idx2word))
        self.embedding_matrix = get_pretrained_embedding(self.idx2word)
...
def get_pretrained_embedding(word_index):
    # read word embedding files
    EMBEDDING_PATH = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
    EMBEDDING_PATH = os.path.join(EMBEDDING_PATH, 'embedding')
    EMBEDDING_FILE = 'glove.6B.50d.txt'
    EMBEDDING_DIM = 50
    
    embeddings_index = {}
    f = open(os.path.join(EMBEDDING_PATH, EMBEDDING_FILE),'r',encoding="utf8")
    for line in f:
        values = line.split()
        word = values[0]
    ...
    f.close()
    EMBEDDING_VOCAB = len(embeddings_index)
    print('Found %s word vectors.' % EMBEDDING_VOCAB)
    # generate embedding matrix
    embedding_matrix = np.zeros([word_index.size,EMBEDDING_DIM])
    for i in range(word_index.size):
        word = word_index[i]
