...
from keras.layers import Input, Embedding, TimeDistributed, Dense, Dropout, Reshape, Concatenate, LSTM, Conv2D, MaxPooling2D, BatchNormalization,Merge
...
def LSTMCNN(opt,embedding_matrix):
    ...
    if opt.use_words:
        ...
        embedding_layer = Embedding(input_dim = opt.word_vocab_size,
                    output_dim = 300,
                    weights=[embedding_matrix],
                    input_length=opt.seq_length,
                    trainable=False)
        word_vecs = embedding_layer(word)       
        
        if opt.batch_norm:
            word_vecs = BatchNormalization()(word_vecs)

        for l in range(opt.highway_layers):
            word_vecs = TimeDistributed(Highway(activation='relu'))(word_vecs)

        for l in range(opt.num_layers):
            word_vecs = LSTM(opt.rnn_size, activation='tanh', recurrent_activation='sigmoid', return_sequences=True, stateful=True)(word_vecs)

            if opt.dropout > 0:
                word_vecs = Dropout(opt.dropout)(word_vecs)
        output_word_vecs = word_vecs
    ...
    if opt.use_chars:
        ...
        if opt.use_words:
            ...
            x = cnn
            inputs = chars
    output = Concatenate()([output_x, output_word_vecs])
    output = TimeDistributed(Dense(opt.word_vocab_size, activation='softmax'))(output)
